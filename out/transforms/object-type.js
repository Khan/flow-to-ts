...
exports.ObjectTypeIndexer = exports.ObjectTypeProperty = exports.ObjectTypeCallProperty = exports.QualifiedTypeIdentifier = exports.ObjectTypeAnnotation = void 0;
...
exports.ObjectTypeAnnotation = {
    ...
    exit(path) {
        ...
        if (spreads.length === 0 &&
            ...
            const value = indexer.typeAnnotation.typeAnnotation;
            const key = indexer.parameters[0].typeAnnotation.typeAnnotation;
            const record = t.tsTypeReference(t.identifier("Record"), t.tsTypeParameterInstantiation([key, value]));
            if (indexer.readonly) {
                path.replaceWith(t.tsTypeReference(t.identifier("Readonly"), t.tsTypeParameterInstantiation([record])));
            }
            else {
...
exports.ObjectTypeCallProperty = {
    exit(path, state) {
        ...
        const replacement = t.tsCallSignatureDeclaration(typeParameters, parameters, typeAnnotation);
        replacement.leadingComments = leadingComments;
        replacement.trailingComments = trailingComments;
        replacement.loc = loc;
        util_js_1.trackComments(replacement, state);
        path.replaceWith(replacement);
    },
};
exports.ObjectTypeProperty = {
    exit(path, state) {
        const { key, value, optional, variance, kind, method, leadingComments, trailingComments, loc, } = path.node; // TODO: static, kind
        ...
        if (method) {
            ...
            util_js_1.trackComments(methodSignature, state);
            ...
            util_js_1.trackComments(propertySignature, state);
...
exports.ObjectTypeIndexer = {
    exit(path, state) {
        const { id, key, value, variance, leadingComments, trailingComments, loc, } = path.node;
        const readonly = variance && variance.kind === "plus";
        if (variance && variance.kind === "minus") {
            // TODO: include file and location of infraction
            console.warn("typescript doesn't support writeonly properties");
        ...
        util_js_1.trackComments(indexSignature, state);
